--@noindex
--@description Pattern generator user interface gui presets
--@author gaspard
--@about User interface presets used in gaspard_Pattern generator.lua script
...
function presets_window.Show()
    ...
    if presets_visible then
        ...
        if reaper.ImGui_Button(ctx, "SAVE##presets_save", 80) then
            --System.SavePreset(preset_search, preset)
            ResetVariables()
        end
